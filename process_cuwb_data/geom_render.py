...
def fetch_geoms_2d(
    ...
    if smooth:
        ...
        for device_id in df_position['device_id'].unique().tolist():
            df_positions_for_device = df_position.loc[
                    df_position['device_id'] == device_id
                ].copy().sort_index()

            df_positions_for_device['x'] = position_filter.filter(
                series=df_positions_for_device['x']
            )
            df_positions_for_device['y'] = position_filter.filter(
                series=df_positions_for_device['y']
            )
    ...
    camera_calibrations = fetch_camera_calibrations(
        camera_ids=camera_info_df.index.unique().to_list(),
        start=start_time,
        end=end_time
    )
...
def create_geom_collection_3d(
    ...
    for (device_id, device_serial_number, entity_type, person_id, person_anonymized_short_name, material_id, material_name), group_df in df.fillna('NA').groupby([
        'device_id',
        'device_serial_number',
        'entity_type',
        'person_id',
...
def project_onto_camera_views(
    ...
    logger.info('Creating 2D geoms from 3D geom, one for each camera: {}'.format(
        camera_info_df['device_name'].to_dict()
    ))
    geom_2d_dict = dict()
